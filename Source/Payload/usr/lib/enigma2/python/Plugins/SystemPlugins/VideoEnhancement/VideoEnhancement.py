...
class VideoEnhancement:
	...
	def createConfig(self, *args):
		...
		# 3798mv200 driver default appears enabled; must set 0 to disable
		...
		# 3798mv200 driver default is enabled; must set to 0 to disable
		...
		if os_path.exists("/proc/stb/vmpeg/0/pep_auto_flesh") and not patchHisi:
		...
		if os_path.exists("/proc/stb/vmpeg/0/pep_dynamic_contrast") and not patchHisi:
							# nonfunctional on 3798mv200
		...
		if 	(os_path.exists("/proc/stb/video/hdmi_colorspace") and
			os_path.exists("/proc/stb/video/hdmi_colorspace_choices") and
			not patchGeneral):
