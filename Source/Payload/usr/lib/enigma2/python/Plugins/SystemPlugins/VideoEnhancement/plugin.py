...
class VideoEnhancementSetup(Screen, ConfigListScreen):
	...
	def createSetup(self):
		...
		self.splitEntry = addToConfigList(_("Split preview mode"), config.pep.split, _("Allows you to view the old and new settings side by side."), True)
		add_to_xtdlist = self.splitEntry is not None
		
		if (patchGeneral):
			
			self.contrastEntry = addToConfigList(_("Peak brightness"), config.pep.contrast, _("Sets the peak brightness level of the video signal."))

			self.brightnessEntry = addToConfigList(_("Black level"), config.pep.brightness, _("Sets the black level of the video signal."))

			self.saturationEntry = addToConfigList(_("Saturation"), config.pep.saturation, _("Adjusts the colourfulness of pixels relative to their brightness."))

			self.hueEntry = addToConfigList(_("Hue"), config.pep.hue, _("Rotates the hue of all colours around an axis corresponding to the colour white."))

			self.sharpnessEntry = addToConfigList(_("Sharpness"), config.pep.sharpness, _("Enhances image detail by adjusting the brightness of pixels relative to their neighbours."), add_to_xtdlist)

			self.scaler_sharpnessEntry = addToConfigList(_("Scaler sharpness"), config.av.scaler_sharpness, _("Enhances image detail when scaling the image to different resolutions."))

			self.scaler_vertical_dejaggingEntry = addToConfigList(_("Scaler vertical dejagging"), config.pep.scaler_vertical_dejagging, _("Smooths jagged edges when scaling the image to different resolutions."))	

			self.block_noise_reductionEntry = addToConfigList(_("Block noise reduction"), config.pep.block_noise_reduction, _("Reduces the appearance of large squares caused by video compression."), add_to_xtdlist)

			self.mosquito_noise_reductionEntry = addToConfigList(_("Mosquito noise reduction"), config.pep.mosquito_noise_reduction, _("Reduces the appearance of small grainy particles around high contrast edges caused by video compression."), add_to_xtdlist)

			self.digital_contour_removalEntry = addToConfigList(_("Digital contour removal"), config.pep.digital_contour_removal, _("Reduces the appearance of coarse steps in colour gradients."), add_to_xtdlist)

			self.smoothEntry = addToConfigList(_("Smoothing"), config.pep.smooth, _("Enhances image smoothness.")) # no idea

			self.dynamic_contrastEntry = addToConfigList(_("Dynamic contrast"), config.pep.dynamic_contrast, _("Adjusts the brightness of light and dark colours to increase image contrast."), add_to_xtdlist)

			self.auto_fleshEntry = addToConfigList(_("Auto flesh"), config.pep.auto_flesh, _("Increases the saturation of colours in the range of flesh tones."), add_to_xtdlist)

			self.blue_boostEntry = addToConfigList(_("Boost blue"), config.pep.blue_boost, _("Increases the saturation of colours in the range of blues."), add_to_xtdlist)

			self.green_boostEntry = addToConfigList(_("Boost green"), config.pep.green_boost, _("Increases the saturation of colours in the range of greens."), add_to_xtdlist)

			self.color_spaceEntry = addToConfigList(_("Color space"), config.pep.color_space, _("Sets the encoding scheme used to define colour subcomponents."))
