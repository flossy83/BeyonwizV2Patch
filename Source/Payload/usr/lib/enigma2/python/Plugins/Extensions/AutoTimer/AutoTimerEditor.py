...
def addAutotimerFromEvent(session, evt = None, service = None):
	...
	if patchGeneral:
		legalChars = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ:()[]&!?-' "
		defaultPath = Directories.resolveFilename(Directories.SCOPE_HDD)
		recordPath = defaultPath
		for c in match:
		  if c in legalChars: recordPath += c
		if recordPath != defaultPath: recordPath += "/" 
		log("addAutotimerFromEvent() recordPath: " + recordPath)
	else: recordPath = None
