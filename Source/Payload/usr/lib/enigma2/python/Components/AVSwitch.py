from config import config, ConfigSlider, ConfigSelection, ConfigSubDict
from config import ConfigYesNo, ConfigEnableDisable, ConfigSubsection 
from config import ConfigBoolean, ConfigSelectionNumber, ConfigNothing, NoSave
...
class AVSwitch:
	...
	def setMode(self, port, mode, rate, force=None):
		log("setMode(%s,%s,%s)" % (port,mode,rate))
		...
		if (patchHisi):
			...
			previous = config.av.hdmicolorspace.value
			config.av.hdmicolorspace.value = cycler[config.av.hdmicolorspace.value]
			# time.sleep(0.05)
			config.av.hdmicolorspace.value = previous
	...
	def setAspect(self, cfgelement, caller=""):
		print "[VideoHardware] setting aspect: %s" % cfgelement.value
		if (caller):
			log("%ssetAspect() setting aspect %s" % (caller, cfgelement.value))
	...
	def setPolicy43(self, cfgelement, cycle=False, caller=""):
		
		# cycle aspect ratio to workaround driver bug
		if (patchHisi) and (cycle):
			
			try:
				modes = ["letterbox", "bestfit", "panscan", "auto"]
				f = open("/proc/stb/video/policy")
				current_aspect = f.readline().strip()
				f.close()
				if current_aspect in modes: modes.remove(current_aspect)
				if cfgelement.value in modes: modes.remove(cfgelement.value)		
				cycle_aspect = modes[0]

				log("%ssetPolicy43() cycling aspect from %s to %s to %s" \
					% (caller, cfgelement.value, cycle_aspect, cfgelement.value))
				f = open("/proc/stb/video/policy", "w")
				f.write(cycle_aspect)
				f.close()
				# time.sleep(0.05)
				f = open("/proc/stb/video/policy", "w")
				f.write(cfgelement.value)
				...
				log("%ssetPolicy43() failed to set aspect ratio")
	
		else:	
			print "[VideoHardware] setting policy: %s" % cfgelement.value
			if (caller):
				log("%ssetPolicy43() setting aspect %s" % (caller, cfgelement.value))
			f = open("/proc/stb/video/policy", "w")
			f.write(cfgelement.value)
			f.close()

	def setPolicy169(self, cfgelement, cycle=False, caller=""):
		
		# cycle aspect ratio to workaround driver bug
		if (patchHisi) and (cycle):				
			
			try:			
				modes = ["letterbox", "scale", "panscan", "auto"]
				f = open("/proc/stb/video/policy2")
				current_aspect = f.readline().strip()
				f.close()
				if current_aspect in modes: modes.remove(current_aspect)
				if cfgelement.value in modes: modes.remove(cfgelement.value)		
				...
				log("%ssetPolicy169() cycling aspect from %s to %s to %s" \
					% (caller, cfgelement.value, cycle_aspect, cfgelement.value))
				...
				log("%ssetPolicy169() failed to set aspect ratio")
				
		else:
			if os.path.exists("/proc/stb/video/policy2"):
				print "[VideoHardware] setting policy2: %s" % cfgelement.value
				if (caller):
					log("%ssetPolicy169() setting aspect %s" % (caller, cfgelement.value))
				f = open("/proc/stb/video/policy2", "w")
				f.write(cfgelement.value)
				f.close()
...
def InitAVSwitch():
	...
	config.av.autores_label_timeout = ConfigSelection(default="8", choices=[("0", _("Not Shown"))] + choicelist)
	config.av.autores_delay = ConfigSelectionNumber(min=500, max=15000, stepwidth=500, default=1000, wraparound=True)
	
	if (patchGeneral):
		config.av.autores_detectionlevel = ConfigSelection(choices=["Normal", "High"], default = "Normal")
		if (patchHisi): config.av.autores_detectionlevel.value = "High"
	...
	if iAVSwitch.supports2160p:	# 2160 moved to end of list to prevent it becoming default
								# which can cause black screen lockout on non-4k TV's
		conv_60 = ["1080p", "1080i", "720p", "480p", "480i", "2160p"]	
		conv_50 = ["1080p50", "1080i50", "720p50", "576p", "576i", "2160p50"] + conv_60	
		conv_30 = ["1080p30", "2160p30"] + conv_60
		conv_24 = ["1080p24", "2160p24"] + conv_60	
		
		if (patchGeneral):			
			conv_60 = ["2160p60", "1080p60", "1080i60", "720p60", "480p60", "480i60"]
			conv_50 = ["2160p50", "1080p50", "1080i50", "720p50", "576p50", "576i50"]
			conv_30 = ["2160p30", "1080p30"] + conv_60
			conv_24 = ["2160p24", "1080p24"] + conv_60
			autores_defaults = {
			"sd24":"1080p24", 
			#"sd25":"576p",		# 25/30Hz were never detected by autores (new_rate always = 24/50/60)
			#"sd30":"480p",		# and there is no disadvantage to outputting them at 50/60Hz
			"sd50i":"576p50",
			"sd50p":"576p50",
			"sd60i":"480p60",
			"sd60p":"480p60",					
		...
		if (patchGeneral):
			conv_60 = ["1080p60", "1080i60", "720p60", "480p60", "480i60"]
			conv_50 = ["1080p50", "1080i50", "720p50", "576p50", "576i50"]
			conv_30 = ["1080p30"] + conv_60
			conv_24 = ["1080p24"] + conv_60
			autores_defaults = {
			"sd24":"1080p24",
		...
		if (patchGeneral):
			conv_60 = ["1080i60", "720p60", "480p60", "480i60"]
			conv_50 = ["1080i50", "720p50", "576p50", "576i50"]
			conv_30 = ["1080p30"] + conv_60
			conv_24 = ["1080p24"] + conv_60
			autores_defaults = {
			"sd24":"1080p24", 
	...
	if (patchGeneral):	
		...
		config.av.autores_sd24 = ConfigSelection(choices=conv_24, default = autores_defaults["sd24"])		
		config.av.autores_sd50i = ConfigSelection(choices=conv_50, default = autores_defaults["sd50i"])
		config.av.autores_sd50p = ConfigSelection(choices=conv_50, default = autores_defaults["sd50p"])
		config.av.autores_sd60i = ConfigSelection(choices=conv_60, default = autores_defaults["sd60i"])
		config.av.autores_sd60p = ConfigSelection(choices=conv_60, default = autores_defaults["sd60p"])	
		config.av.autores_ed24 = ConfigSelection(choices=conv_24, default = autores_defaults["ed24"])	
		config.av.autores_ed50 = ConfigSelection(choices=conv_50, default = autores_defaults["ed50"])
		config.av.autores_ed60 = ConfigSelection(choices=conv_60, default = autores_defaults["ed60"])
		config.av.autores_hd24 = ConfigSelection(choices=conv_24, default = autores_defaults["hd24"])		
		config.av.autores_hd50i = ConfigSelection(choices=conv_50, default = autores_defaults["hd50i"])
		config.av.autores_hd50p = ConfigSelection(choices=conv_50, default = autores_defaults["hd50p"])
		config.av.autores_hd60i = ConfigSelection(choices=conv_60, default = autores_defaults["hd60i"])	
		config.av.autores_hd60p = ConfigSelection(choices=conv_60, default = autores_defaults["hd60p"])
		config.av.autores_uhd24 = ConfigSelection(choices=conv_24, default = autores_defaults["uhd24"])
		config.av.autores_uhd50 = ConfigSelection(choices=conv_50, default = autores_defaults["uhd50"])
		config.av.autores_uhd60 = ConfigSelection(choices=conv_60, default = autores_defaults["uhd60"])
			
		# Force defaults for these redundant settings to have configlist.py delete them from file	
		config.av.autores_sd25 = ConfigSelection(choices=conv_50, default = "576p50")
		config.av.autores_sd25.value = "576p50"
		config.av.autores_sd25.save()	
		config.av.autores_sd30 = ConfigSelection(choices=conv_30, default = "480p60")
		config.av.autores_sd30.value = "480p60"
		config.av.autores_sd30.save()			
		config.av.autores_ed25 = ConfigSelection(choices=conv_50, default = "720p50")
		config.av.autores_ed25.value = "720p50"
		config.av.autores_ed25.save()	
		config.av.autores_ed30 = ConfigSelection(choices=conv_30, default = "720p60")
		config.av.autores_ed30.value = "720p60"
		config.av.autores_ed30.save()	
		config.av.autores_hd25 = ConfigSelection(choices=conv_50, default = "1080p50")
		config.av.autores_hd25.value = "1080p50"
		config.av.autores_hd25.save()	
		config.av.autores_hd30 = ConfigSelection(choices=conv_30, default = "1080p60")
		config.av.autores_hd30.value = "1080p60"
		config.av.autores_hd30.save()	
		config.av.autores_hd50 = ConfigSelection(choices=conv_50, default = "1080p50")
		config.av.autores_hd50.value = "1080p50"
		config.av.autores_hd50.save()			
		config.av.autores_hd60 = ConfigSelection(choices=conv_60, default = "1080p60")
		config.av.autores_hd60.value = "1080p60"
		config.av.autores_hd60.save()	
		config.av.autores_uhd25 = ConfigSelection(choices=conv_50, default = "2160p50")
		config.av.autores_uhd25.value = "2160p50"
		config.av.autores_uhd25.save()		
		config.av.autores_uhd30 = ConfigSelection(choices=conv_30, default = "2160p60")
		config.av.autores_uhd30.value = "2160p60"
		config.av.autores_uhd30.save()
	...
	if (patchHisi):
		config.av.aspect = ConfigSelection(choices={
			"4:3": _("4:3"),
			"16:9": _("16:9"),
			# "auto": _("Automatic"), # nonfunctional
			"16:10": _("16:10")},
			default="16:9")
	else:
		config.av.aspect = ConfigSelection(choices={
	...
	if (patchHisi):
		# proc/stb/video/policy2 appears to be nonfunctional,
		# but we'll force it to "original aspect ratio" value just to be sure
	...
	if (patchHisi):	
		policy_choices = {	
			"panscan": _("Zoom to fill screen"),
			"letterbox": _("Original aspect ratio"),	
			"bestfit": _("Stretch to fill screen")
		}
	else:
		policy_choices = {
	...
	if have_colorspace:
		def setHDMIColorspace(configElement):
			log("setHDMIColorspace(%s)" % configElement.value)
			try:
		...
		if getBoxType() in ('vusolo4k','vuuno4k','vuultimo4k'):
			...
			if (patchHisi):
				config.av.hdmicolorspace = ConfigSelection(choices={
					#"auto": _("Auto"), # auto outputs RGB limited range for SDR
										# which many displays are not good at handling
					
					"rgb": _("RGB"),	# bad colour - incorrect luma coefficients?
					#"420": _("420"),	# nonfunctional
					"422": _("YCbCr 4:2:2"),
					"444": _("YCbCr 4:4:4")},
					default = "444") 	
			else:
	...
	if have_HdmiHdrType:
		def setHdmiHdrType(configElement):		
			try:
				#log("setHdmiHdrtype(%s)" % configElement.value)
